...
import com.ycs.gulimall.constant.AuthConstant;
import com.ycs.gulimall.exception.BizCodeEnum;
import com.ycs.gulimall.feign.MemberFeignService;
import com.ycs.gulimall.feign.ThirdPartFeignService;
...
public class LoginController {
    ...
    public String register(@Valid UserRegisterVo vos, BindingResult result, RedirectAttributes attributes) {
        ...
        //获取存入Redis里的验证码
        String redisCode = stringRedisTemplate.opsForValue().get(AuthConstant.SMS_CODE_CACHE_PREFIX + vos.getPhone());
        if (!StringUtils.isEmpty(redisCode)) {
            //截取字符串
            if (code.equals(redisCode.split("_")[0])) {
                //删除验证码;令牌机制
                stringRedisTemplate.delete(AuthConstant.SMS_CODE_CACHE_PREFIX+vos.getPhone());
